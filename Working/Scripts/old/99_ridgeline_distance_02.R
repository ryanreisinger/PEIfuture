...
s85$scenario <- "rcp85"
s <- rbind(s45, s85)
s[s$when == "present", "scenario"] <- "Current"
...
for (i in spNamesWinter) {
    ...
    w85$scenario <- "rcp85"
    w <- rbind(w45, w85)
    w[w$when == "present", "scenario"] <- "Current"
...
dists_thr <- left_join(x = dists, y = tst, by = c("species", "season", "model", "scenario"), keep = FALSE) %>%
...
ggplot(data = dists_thr, aes(x = distance, y = model, fill = scenario, colour = scenario)) +
...
ggplot(data = dists_thr, aes(x = distance, y = scenario, fill = scenario, colour = scenario)) +
