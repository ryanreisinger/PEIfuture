...
for (m in all.climate.mods) {
  ...
  for (i in c("summer", "winter")) {
    ...
    pdf(paste0("./Data/dendrogram_plots_bakers/bakers", "_", "_", this.climate, "_", this.season, ".pdf"),
        height = 5, width = 5)
    plot(density(cor_bakers_gamma_results),
         main = paste0("Baker's gamma distribution under H0\n", this.climate, " - ", this.season),
         xlim = c(-1,1))
    abline(v = 0, lty = 2)
    ...
    pdf(paste0("./Data/dendrogram_plots_tanglegram/tanglegram", "_", this.climate, "_", this.season, ".pdf"),
        height = 3.5,
        width = 4.5)
    tanglegram(dl,
               sort = TRUE,
