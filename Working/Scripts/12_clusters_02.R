...
for (m in all.climate.mods) {
  ...
  for (i in c("summer", "winter")) {
    ...
    hist_cons <- recluster.cons(hist.dist, method = "average")$cons # Consensus tree
    ...
    # hist_cc <- cor(hist.dist, hist.coph, use = "complete.obs")
    hist_cc <- cor_cophenetic(hist_cons, hist.dist)
    ...
    fut_cons <- recluster.cons(fut.dist, method = "average")$cons # Consensus tree
    
    #get the CCC to compare methods - correlation between distance matrix and the clustering matrix
    # fut.coph <- cophenetic(fut.clust)
    # fut_cc <- cor(fut.dist, fut.coph)
    fut_cc <- cor_cophenetic(fut_cons, fut.dist)
    ...
    if (TRUE) {
    foo <- dist(x = t(hold.hist), method = "canberra")
      foo <- recluster.cons(foo, method = "average")$cons
      dHis <- as.dendrogram(foo)
    
    foo <- dist(x = t(hold.fut), method = "canberra")
      foo <- recluster.cons(foo, method = "average")$cons
      dFut <- as.dendrogram(foo)
    } else {
    ...
    # Bakers - TIFF
    tiff(paste0("./Data/dendrogram_plots_bakers/bakers", "_", "_", this.climate, "_", this.season, "_", which_scenario, ".tiff"),
    ...
    # Label
    lab <- paste(paste0("R = ", round(the_cor, 2), ". p = ",  round(sum(the_cor < cor_bakers_gamma_results)/ R, 4), "\n"),
                  paste0("Current CCC = ", round(hist_cc, 2), "\n"),
                  paste0("Future CCC = ", round(fut_cc, 2)),
                 sep = "")
    ...
    mtext(lab, side = 1, line = 1, outer = TRUE, cex = 0.8)
    dev.off()
    
    # Tanglegram - TIFF
    tiff(paste0("./Data/dendrogram_plots_tanglegram/tanglegram", "_", this.climate, "_", this.season, "_", which_scenario, ".tiff"),
        height = 3.5,
        width = 4.5, units = "in",
        res=300, compression = 'lzw')
    par(oma = c(4, 0, 0, 0))
    ...
    this_frame <- data.frame("season" = this.season,
                             "scenario" = which_scenario,
                             "model" = this.climate,
                             "tangle_R" = round(the_cor, 2),
                             "p" = round(sum(the_cor < cor_bakers_gamma_results)/ R, 4)
    )
    
    hold <- rbind(hold, this_frame)
    
  }
  
  master <- rbind(master, hold)
  
}

write.csv(master, paste0("./Data/dendrogramResults/dendrogram_results_", which_scenario, ".csv"), row.names = F)
...
for (j in c("summer", "winter")) {
  these_files <- list.files("./Data/dendrogram_plots_bakers/", pattern="*.tiff", full.names = T)
  these_files <- these_files[grep(which_scenario, these_files)]
  these_files <- these_files[grep(j, these_files)]
  ...
  tiff(paste0("./Data/compiledPlots/dendrogram_", j, "_", which_scenario, ".tiff"), units="in", width=6, height=10, res=300, compression = 'lzw')
  par(mfrow = c(4, 2), mai = c(0,0,0,0), oma = c(0, 0.5, 3, 0))
  for (i in these_files) {
    this <- image_read(i)
    plot(this)
  }
  mtext(paste0(season_label, " - ", scenario_label), side = 3, line = 1, outer = TRUE, adj = 0, cex = 0.8)
  dev.off()
...
# Tanglegram
for (j in c("summer", "winter")) {
these_files <- list.files("./Data/dendrogram_plots_tanglegram/", pattern="*.tiff", full.names = T)
these_files <- these_files[grep(which_scenario, these_files)]
these_files <- these_files[grep(j, these_files)]
...
tiff(paste0("./Data/compiledPlots/tanglegram_", j, "_", which_scenario, ".tiff"), units="in", width=8.3, height=10, res=300, compression = 'lzw')
par(mfrow = c(4, 2), mai = c(0,0,0,0), oma = c(0, 0.5, 3, 0))
for (i in these_files) {
  this <- image_read(i)
  plot(this)
